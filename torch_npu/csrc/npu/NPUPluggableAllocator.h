...
struct NPUPluggableAllocator
    ...
    // Pool / NPU-graph interaction callbacks.  If not registered the methods
    // are silent no-ops so that pluggable allocators (e.g. SHMEM) that manage
    // their own memory can still be used together with NPU graph capture.
    void set_begin_allocate_to_pool_fn(
        std::function<void(c10::DeviceIndex, c10_npu::MempoolId_t,
                           std::function<bool(aclrtStream)>)>
            fn);
